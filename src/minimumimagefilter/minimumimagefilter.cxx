...
int main( int argc, char **argv )
{
	...
	if( argc != 3 && argc != 4 )
	{
		...
		std::cerr << "pxminimumimagefilter image1 image2 [outputname]" << std::endl;
		std::cerr << "This program only accepts 2D short images." << std::endl;
		return 1;
	}
	
	/** Print arguments. */
	std::cout << "pxminimumimagefilter ";
  ...
  typedef itk::ImageFileReader< ImageType >							ReaderType;
	...
	typedef itk::MinimumImageFilter<
		ImageType, ImageType, ImageType >										MinimumFilterType;
  ...
  /** Now minimum the two images. */
	MinimumFilterType::Pointer minimumFilter = MinimumFilterType::New();
	minimumFilter->SetInput1( image1Reader->GetOutput() );
	minimumFilter->SetInput2( image2Reader->GetOutput() );
