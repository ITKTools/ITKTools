...
/** Declare ComputeDifferenceImage. */
template< class OutputPixelType, unsigned int Dimension >
void ComputeDifferenceImageScalar( std::string inputFileName1,
...
int main( int argc, char **argv )
{
  ...
  /** TASK 3:
    ...
    {
      run(ComputeDifferenceImageScalar,unsigned char,2);
      run(ComputeDifferenceImageScalar,char,2);
      run(ComputeDifferenceImageScalar,unsigned short,2);
      run(ComputeDifferenceImageScalar,short,2);
      run(ComputeDifferenceImageScalar,int,2);
      run(ComputeDifferenceImageScalar,unsigned int,2);
      run(ComputeDifferenceImageScalar,long,2);
      run(ComputeDifferenceImageScalar,unsigned long,2);
      run(ComputeDifferenceImageScalar,float,2);
      run(ComputeDifferenceImageScalar,double,2);

      run(ComputeDifferenceImageScalar,unsigned char,3);
      run(ComputeDifferenceImageScalar,char,3);
      run(ComputeDifferenceImageScalar,unsigned short,3);
      run(ComputeDifferenceImageScalar,short,3);
      run(ComputeDifferenceImageScalar,int,3);
      run(ComputeDifferenceImageScalar,unsigned int,3);
      run(ComputeDifferenceImageScalar,long,3);
      run(ComputeDifferenceImageScalar,unsigned long,3);
      run(ComputeDifferenceImageScalar,float,3);
      run(ComputeDifferenceImageScalar,double,3);

      run(ComputeDifferenceImageScalar,unsigned char,4);
      run(ComputeDifferenceImageScalar,char,4);
      run(ComputeDifferenceImageScalar,unsigned short,4);
      run(ComputeDifferenceImageScalar,short,4);
      run(ComputeDifferenceImageScalar,int,4);
      run(ComputeDifferenceImageScalar,unsigned int,4);
      run(ComputeDifferenceImageScalar,long,4);
      run(ComputeDifferenceImageScalar,unsigned long,4);
      run(ComputeDifferenceImageScalar,float,4);
      run(ComputeDifferenceImageScalar,double,4);
    } // end scalar support
    /**
	...
	/**
	 * ******************* ComputeDifferenceImageScalar *******************
	 *
	 * The ComputeDifferenceImage function templated over the output pixel type.
	 */
  ...
  std::string inputFileName2, std::string outputFileName )
{
  ...
  typedef itk::ConstrainedValueDifferenceImageFilter<
    DoubleImageType, DoubleImageType, OutputImageType > DifferenceFilterType;
  ...
  differenceFilter->SetInput2( reader2->GetOutput() );
...
void PrintHelp()
{
	std::cout << "Usage:" << std::endl << "pxcomputedifferenceimage" << std::endl;
	std::cout << "  -in      inputFilename1 inputFilename2" << std::endl;
	std::cout << "  [-out]   outputFilename, default in1MINUSin2.mhd" << std::endl;
